...
from .models import stipendRequest
...
def adminlogin(request):
    if request.method=="POST":
        ...
        try:
            user = coordinator.objects.get(coid=username,pwd=password)
            context={'admin':user.id}
...
def StiReq(request, id):
    ...
    if obj.status == 'Not Assigned':
        ...
        if request.method=="POST":
            ...
            StiReq = stipendRequest(Month = Month, TAObj= obj, CourseObj= cours, FacultyObj=fac, message=text)
            StiReq.save()
...
def viewCourse(request, id):
    obj = ta.objects.get(id=id)
    if obj.status == 'Not Assigned':
        context = {'taid' : obj.id, 'user':obj, 'message': "You are not assigned to any course yet!"}
...
def seeReq(request, id):
    obj = faculty.objects.get(id=id)
    reqs = stipendRequest.objects.filter(FacultyObj = obj)
    context = {'facid':obj.id, 'reqs':reqs}
    return render(request, 'seeReq.html', context)
